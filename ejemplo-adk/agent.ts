...
const getWeather = new FunctionTool({
  ...
  execute: async ({ city }) => {
    ...
    if (!data) {
      return { error: `No hay datos para "${city}". Ciudades disponibles: ${Object.keys(MOCK_WEATHER).join(', ')}` };
    }
...
function inputGuardrail(callbackContext: unknown, llmRequest: { contents?: Array<{ parts?: Array<{ text?: string }> }> }) {
  const lastMessage =
    llmRequest.contents?.at(-1)?.parts?.at(-1)?.text ?? '';

  const forbidden = ['contraseña', 'password', 'token', 'credit card', 'tarjeta'];
  const hasForbidden = forbidden.some(word =>
    lastMessage.toLowerCase().includes(word)
  );

  if (hasForbidden) {
    console.warn('[guardrail:input] Petición bloqueada — contiene términos prohibidos');
    return {
      candidates: [{
        content: {
...
function outputGuardrail(callbackContext: unknown, llmResponse: { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> }) {
  const text =
    llmResponse.candidates?.at(0)?.content?.parts?.at(0)?.text ?? '';

  // Ejemplo: ocultar patrones que parezcan emails en la respuesta
  const sanitized = text.replace(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, '[email redactado]');
...
export const rootAgent = new LlmAgent({
  ...
  description: 'Agente de demostración que informa sobre el tiempo en ciudades españolas.',
  instruction: `
